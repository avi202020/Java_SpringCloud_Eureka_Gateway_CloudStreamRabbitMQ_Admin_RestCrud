...
# Introduction
Micro-service application with Eureka, Gateway, Spring Cloud Stream using RabbitMQ, Admin, and Rest Crud.

# Understanding the application
For the Rest part it is the same as the dozens of apps I have already published, /api/todos will fetch the todos etc.
Once you create a Todo with POST /api/todos the todo-service will feed the Todo into the Cloud Stream(I use RabbitMQ as the binder but yoyu could use Kafka),
the receiver will be the admin-server-service, the admin-server-service is receiver and sender, hence it is a Processor,
once received it will send a string to the cloud stream, that notification string will be received by listener-service.
In few words:
Output for todo-service is the Input for admin-server-service, Output of admin-server-service is
the input for listener-service, with this in mind, understanding the yml files should be easy.
